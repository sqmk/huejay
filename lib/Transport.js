...
class Transport {
  ...
  sendRequest(options) {
    options = Object.assign(
      {
        host:   `http://${this.client.host}:${this.client.port}`,
        method: 'GET',
        path:   '',
        query:  {},
        body:   {},
    ...
    return new Promise(
      (resolve, reject) => {
        request(
          ...
          (error, response, body) => {
            return this.handleResponse(options, resolve, reject, error, response, body);
          }
        );
      }
    );
   ...
   */
  handleResponse(options, resolve, reject, error, response, body) {
    if (error) {
      return reject(
        new Error({
          description: 'Failed to make the request: ' + error
        })
      );
    }
    ...
    // Immediately return result if raw toggled
    if (options.raw) {
        ...
        if (result[i].error !== undefined) {
          return reject(
            new Error({
              ...
              description: result[i].error.description
            })
          );
        }
