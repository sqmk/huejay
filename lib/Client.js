...
class Client {
   ...
   */
  getRules() {
    let GetRules = require('./Command/GetRules');

    return this.invokeCommand(new GetRules);
  }

  /**
   * Get rule
   *
   * @param {mixed} ruleId Rule Id
   *
   * @return {Promise} Promise for chaining
   */
  getRule(ruleId) {
    let GetRule = require('./Command/GetRule');

    return this.invokeCommand(new GetRule(ruleId));
  }

  /**
   * Delete rule
   *
   * @param {mixed} rule Rule object or rule id
   *
   * @return {Promise} Promise for chaining
   */
  deleteRule(rule) {
    let DeleteRule = require('./Command/DeleteRule');

    return this.invokeCommand(new DeleteRule(rule));
  }
